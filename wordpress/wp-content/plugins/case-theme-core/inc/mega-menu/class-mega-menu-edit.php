...
//    private $extra_menu= $extra_menu_custom;
    ...
    function get_fields($item, $depth = 0, $args = array(), $id = 0)
    {
        ...
        $this->mega_locations = apply_filters('ct_locations', array('primary'));
        $check_mega = true;
    ...
    <?php if (0 === $depth && $check_mega && $enable_megamenu === true) : ?>
        <p class="description description-wide">
            <label for="edit-menu-item-ct-megaprofile-<?php echo esc_attr($item_id); ?>">
                <?php esc_html_e('Select Mega Menu', CT_TEXT_DOMAIN); ?><br/>
                <select id="edit-menu-item-ct-megaprofile-<?php echo esc_attr($item_id); ?>" class="widefat"
                    ...
                    <option value="0"><?php esc_html_e('None', CT_TEXT_DOMAIN) ?></option>
                    <?php
                    $r = $this->walker_args;
    ...
    <?php //if ($check_mega): ?>
        <p class="description description-wide">
            <label for="edit-menu-item-ct-icon-<?php echo esc_attr($item_id); ?>">
                <?php esc_html_e('Icon', CT_TEXT_DOMAIN); ?><br/>
                <select id="edit-menu-item-ct-icon-<?php echo esc_attr($item_id); ?>"
                        class="widefat ct-icon-picker"
                        name="menu-item-ct-icon[<?php echo esc_attr($item_id); ?>]">
                    <option value="" <?php selected('', esc_attr($item->ct_icon)) ?>><?php esc_html_e('No Icons', CT_TEXT_DOMAIN) ?></option>
                    <?php $arr = $this->ct_iconpicker_fontawesome();
                    foreach ($arr as $group => $icons) { ?>
                        <optgroup label="<?php echo esc_attr($group); ?>">
                            <?php foreach ($icons as $key => $label) {
                                $class_key = key($label); ?>
                                <option value="<?php echo esc_attr($class_key); ?>" <?php selected($class_key, esc_attr($item->ct_icon)) ?>><?php echo esc_html(current($label)); ?></option>
                            <?php } ?>
                        </optgroup>
                    <?php } ?>
                </select>
    ...
    <?php if ($enable_ongpage_option && 0 === $depth) : ?>
        <p class="description description-wide">
            <label for="menu-item-ct-onepage-<?php echo esc_attr($item_id); ?>">
                <?php esc_html_e('One page', CT_TEXT_DOMAIN); ?><br/>
                <select id="menu-item-ct-onepage-<?php echo esc_attr($item_id); ?>"
                        class="widefat menu-item-ct-onepage"
                        name="menu-item-ct-onepage[<?php echo esc_attr($item_id); ?>]">
                    <option value="no-one-page" <?php selected(esc_attr($item->ct_onepage), 'no-one-page') ?>><?php esc_html_e('No', CT_TEXT_DOMAIN) ?></option>
                    <option value="is-one-page" <?php selected(esc_attr($item->ct_onepage), 'is-one-page') ?>><?php esc_html_e('Yes', CT_TEXT_DOMAIN) ?></option>
                </select>
            </label>
        </p>
        <p class="description description-wide">
            <label for="menu-item-ct-onepage-offset-<?php echo esc_attr($item_id); ?>">
                <?php esc_html_e('One page offset', CT_TEXT_DOMAIN); ?><br/>
                <input type="number" min="0" id="menu-item-ct-onepage-offset-<?php echo esc_attr($item_id); ?>"
                        class="widefat menu-item-ct-onepage-offset"
                        name="menu-item-ct-onepage-offset[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr($item->ct_onepage_offset); ?>" />
            </label>
        </p>
    <?php endif;

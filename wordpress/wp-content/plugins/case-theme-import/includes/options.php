...
function ct_ie_options_export($file)
{
    ...
    /* default. */
    $options['home'] = ct_ie_options_get_home_page();
    $options['menus'] = ct_ie_options_get_menus();
    $options['opt-name'] = ct_ie_setting_get_opt_name($file);
    $options['export'] = !empty($_POST['ct-ie-data-type']) ? $_POST['ct-ie-data-type'] : array();

    /* wp options */
    $options['wp-options'] =ct_ie_options_get_wp_options(apply_filters('ct_ie_options_wp_options', array()));
...
function ct_ie_options_get_home_page()
{
...
function ct_ie_options_get_menus()
{
...
function ct_ie_options_get_wp_options($options = array())
{
    if (empty($options))
        return $options;
    ...
    foreach ($options as $key => $option) {
        switch ($key) {
            case 'home':
                ...
                $import_result[] = esc_html__('Set home page successfully!', 'ct-ie');
                break;
            case 'menus':
                ct_ie_options_set_menus($option);
                $import_result[] = esc_html__('Import menus successfully!', CTI_TEXT_DOMAIN);
                break;
            case 'wp-options':
                ct_ie_options_set_wp_options($option);
                $import_result[] = esc_html__('Import wp options successfully!', CTI_TEXT_DOMAIN);
                break;
        }
    }
}

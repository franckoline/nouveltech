...
class CSS_Generator {
	...
	protected function inline_css() {
		...
		if ( ! empty( $header_bg_color ) ) {
			printf( '#ct-header-wrap.ct-header-layout3 #ct-header:not(.h-fixed), #ct-header-wrap.ct-header-layout6 #ct-header:not(.h-fixed), #ct-header-wrap.ct-header-layout8 .ct-header-main:not(.h-fixed) .ct-header-navigation, #ct-header-wrap.ct-header-layout9 #ct-header:not(.h-fixed) { background-color: %s !important; }', esc_attr( $header_bg_color ) );
		}
		...
		if ( ! empty( $topbar_bg_color ) ) {
			printf( '#ct-header-wrap.ct-header-layout1 #ct-header-top, #ct-header-wrap.ct-header-layout6 #ct-header-top { background-color: %s !important; }', esc_attr( $topbar_bg_color ) );
		}

...
$size_list = $widget->get_setting('size_list');
...
$load_more = array(
    ...
    'template_type' => 'portfolio_layout5',
);
?>

<div id="<?php echo esc_attr($html_id) ?>" class="ct-grid ct-portfolio-grid5" data-layout="<?php echo esc_attr($layout_type); ?>" data-start-page="<?php echo esc_attr($paged); ?>" data-max-pages="<?php echo esc_attr($max); ?>" data-total="<?php echo esc_attr($total); ?>" data-perpage="<?php echo esc_attr($limit); ?>" data-next-link="<?php echo esc_attr($next_link); ?>">
    ...
    <?php if ($filter == "true" and $layout_type == 'masonry'): ?>
        <div class="grid-filter-wrap">
            ...
            <?php foreach ($categories as $category): ?>
                ...
                <span class="filter-item" data-filter="<?php echo esc_attr('.ct-term-' . $term->term_id); ?>">
    ...
    <?php if (!empty($next_link) && $layout_type == 'masonry' && $pagination_type == 'loadmore') { ?>
        <div class="ct-load-more text-center <?php echo esc_attr($ct_animate); ?>" data-loadmore="<?php echo esc_attr(json_encode($load_more)); ?>">
